...
# B may run:
#   sw $t0, 16($sp)
# which would store data from $t0 into the parents return value register
...
.text
factorial:
  # base  case - still in parents stack segment
  ...
  # recursive case - move to this calls stack segment
  ...
  lw      $ra, 8($sp)        load this calls $ra again(we just got back from a jump)
  lw      $t1, 4($sp)       # load childs return value into $t1

  lw      $t2, 12($sp)      # load parents start value into $t2
# return x * factorial(x-1) (not the return statement, but the multiplication)
  mul     $t3, $t1, $t2     # multiply childs return value by parents working value, store in $t3.

  sw      $t3, 16($sp)       take result(in $t3), store in parents return value.
...
returnOne:
  ...
  sw      $t0, 4($sp)       # store 1 into the parents return value register
  jr      $ra               # jump to parent call
